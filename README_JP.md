...
*   **効率的なUSBデバイス検出 (macOS)**: USBデバイス（特にOAK-D Lite）の検出にmacOSネイティブのIOKitフレームワークを利用するようになりました。このイベント駆動型アプローチは以前のポーリング方式を置き換え、以下の改善を実現しています：
    *   **効率の向上**: デバイスの接続状態を常に確認する必要がなくなったため、CPU使用率が低減されました。
    *   **応答性の向上**: デバイスの接続・切断がほぼ瞬時に検出されます。
    *   **信頼性の向上**: ポーリングと比較して、様々なシステム状態での検出がより堅牢になりました。
    **注意**: この高度なUSB検出機能は、IOKitを使用しているためmacOS専用です。
...
*   `pyobjc`: macOS上でUSBデバイスイベントを監視するためのネイティブIOKit統合に必要です。macOSでこのアプリケーションを実行する場合は、`pyobjc`がインストールされていることを確認してください（`requirements.txt` に含まれており、上記の`pip install -r requirements.txt`コマンドで一緒にインストールされます）。手動でインストールする場合は、通常 `pip install pyobjc` を実行します。
...
手動での動作確認が主なテスト方法となります。
今後の改善点として、ユニットテストや結合テストの導入を検討しています。
